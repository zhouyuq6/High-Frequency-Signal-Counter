...
/*MACROS*/
int Q0_0=3;                           //Read pins from two LS160 chip
int Q0_1=4;                           //Q0 is least significant chip (LSC)
int Q0_2=5;                           //Q1 is most significant chip (MSC)
int Q0_3=6;
int Q1_0=7;
int Q1_1=8;
int Q1_2=9;
...
int writeCycle=30;                    //Time period in seconds between two USB writing procedure.

/*GLOBAL VARIABLES*/
long SEC=1000000;                     //One second in microseconds. For Timer 1 period setting.
int rb0=0;                            //rb0~rb7 are bits read from Parallel Outputs from two chips
int rb1=0;    
int rb2=0;
int rb3=0;
int rb4=0;
...
int rb6=0;
int rb7=0;
unsigned int count=0;                 //Number of counts. Increment 1 every one hundred counts.
...
void setup() {
  ...
  Timer1.attachInterrupt(timer);                                  //Define Timer1 interrupr ISR: "timer()"
}
/*ISR*/
void timer(){                         //Timer 1
  timing=timing+1;
...
void loop() {
  ...
  if(digitalRead(SW)==LOW){           //Switch botton pressed
    ...
    if(!switchOn){                    
      ...
      else{                           //USB disk is not present or connection to ch376s failed
    ...
    /*END PROCEDURE*/
      ...
      count=0;                        //Clear variables
      timing=0;
      switchOn=!switchOn;             //Procedure is successfully ended
      blinkTwice();
    }
  }
  ...
  if(timing>=writeCycle && switchOn){
    ...
    writeString=writeString+remainCount+",";          //Data are in .CSV format.
...
int remain(){
  rb0=digitalRead(Q0_0);                              //Reads bits of two chips
  ...
  int remainHigh= rb4 + 2*rb5 + 4*rb6 + 8*rb7;        //remainLow is from LSC; remainHigh from MSC.
  if(remainLow==9){                                   //LS160 is a BCD 4bit counter. TC goes high when the count reaches 9.
    remainHigh=remainHigh-1;                          //These operations are for output value correction
  }
  int remainTot=(remainHigh)*10+remainLow;
  if(remainTot==-1){
    remainTot=99;
  }
  return remainTot;
